...
export function ProductPage() {
  ...
  const { data: product, isLoading: loading } = useQuery({
    queryKey: ["catalogue-product", handle],
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
      ...
      <div>
                ...
                {isColor ? (
                  <div className="flex flex-wrap gap-2.5">
                    {opt.values.map((value) => {
                      ...
                      const isLight = [
                        "#ffffff", "#f5f0e1", "#e7d4b5", "#d1d5db", "#facc15", "#84cc16", "#7dd3fc",
                      ].includes(hex);
                      return (
                        <button
                          key={value}
                        ...
                        >
                          {selected && (
                            <Check
                              className={cn("h-4 w-4", isLight ? "text-black" : "text-white")}
                              strokeWidth={3}
                            />
                          )}
                          {!available && (
        ...
        {GANG_BUILDER_URLS[product.handle] && (
          <div className="mt-4 rounded-lg border border-dashed border-primary/40 bg-primary/5 p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Use our Gang Sheet Builder to arrange your artwork before requesting a quote.
            </p>
            <a
...
function RecentlyViewed({ currentHandle }: { currentHandle: string }) {
  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      ...
      return results.filter(Boolean) as CatalogueProduct[];
    },
  });
          ...
          {products.map((p) => (
            ...
            >
              ...
              <p className="mt-1 text-sm font-bold text-primary">
                {p.price.currencyCode} {parseFloat(p.price.amount).toFixed(2)}
              </p>
            </Link>
          ))}
        </div>
