...
export function AdminPage() {
  ...
  async function loadData() {
    ...
    // Fire social webhooks for newly-promoted posts.
    for (const row of (promoted as { id: string }[] | null) ?? []) {
      void supabase.functions.invoke("social-webhook-dispatch", {
        body: { post_id: row.id },
      });
    }
    ...
    if (postsRes.error) {
      toast.error(`Failed to load posts: ${postsRes.error.message}`);
    }

    if (viewsRes.error) {
      toast.error(`Failed to load analytics: ${viewsRes.error.message}`);
    }
  ...
  const filteredViews = useMemo(
    () =>
      views.filter(
        (v) => new Date(v.viewed_at).getTime() >= cutoffMs,
      ),
    [views, cutoffMs],
  );
  ...
  const sortedPosts = useMemo(
    () =>
      [...posts]
        ...
        .sort(
          (a, b) =>
            (viewsByPost.get(b.id) ?? 0) -
            (viewsByPost.get(a.id) ?? 0),
        ),
              ...
              <>
                ...
                <div className="mt-8 overflow-hidden rounded-lg border border-border bg-card">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-muted/50 px-4 py-3">
                    ...
                    <div className="inline-flex rounded-md border border-border bg-card p-1">
                      {(
                        ["published", "scheduled", "draft"] as const
                      ).map((t) => {
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {(() => {
                          const colCount =
                            6 +
                            (blogTab === "published" ? 1 : 0) +
                            (blogTab === "scheduled" ? 1 : 0);
